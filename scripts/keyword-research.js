...
输出JSON格式：{ "topics": [ { "topic": "...", "category": "...", "priority": 1-10, "reason": "..." } ] }`;
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `请生成20个适合好物推荐博客的文章话题。今天是${new Date().toLocaleDateString('zh-CN')}。
...
每个话题标注优先级(1-10)，10为最高。优先考虑当前季节和热点的需求。`,
    ...
    if (jsonMatch) {
      ...
      const newTopics = (data.topics || []).filter(t => !existingTopics.has(t.topic));
