...
async function callClaude(systemPrompt, userMessage, options = {}) {
  ...
  if (!apiKey) {
    ...
    console.error('请设置: export ANTHROPIC_API_KEY=sk-ant-xxx');
    process.exit(1);
  }
...
请输出JSON格式，包含 outline（数组，每项含level/heading/content）、keywords（关键词数组）、internalLinks（内部链接主题数组）、faqQuestions（3-5个FAQ问题）。`;
  ...
  const result = await callClaude(systemPrompt, userMessage, { temperature: 0.6 });
...
请输出JSON：
{
  ...
  try {
    const result = await callClaude(systemPrompt, userMessage, { temperature: 0.4, maxTokens: 2048 });
    const jsonMatch = result.text.match(/\{[\s\S]*\}/);
    if (jsonMatch) {
